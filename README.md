...
2. **多视角图片批量输入节点** (`MultiViewImageBatch`) ⭐ 推荐:
   - 直接接受图片列表（batch）输入
   - 自动处理批量图片
   - 适合视频帧提取、批量生成等场景
   - 只需一根连线，更简洁！

3. **多视角图片输入节点** (`MultiViewImageInput`):
   - 可以连接 1-8 张单独的图片
   - 将多张图片组合成一个多视角数据
   - 适合手动选择不同来源的图片

4. **多视角图片预览节点** (`MultiViewImagePreview`) ⭐ 推荐用于快速预览:
   - 使用 ComfyUI 标准方式预览所有图片
   - 直接在界面显示，无需额外配置
   - 轻量级，加载速度快
   - 适合调试和检查图片
...
```
[图片列表/Batch] ──> [多视角图片批量输入] ──> [多视角图片预览 🖼️]
...
```
[图片列表/Batch] ──> [多视角图片批量输入] ──> [保存3D预览HTML 💾]
...
```
[图片列表/Batch] ──> [多视角图片批量输入] ─┬──> [多视角图片预览] (查看)
                                         └──> [保存3D预览HTML] (导出)
...
如果你的图片已经是**列表格式（batch）**，强烈推荐使用 **多视角图片批量输入** 节点！
...
详细说明请查看 [BATCH_INPUT_README.md](BATCH_INPUT_README.md)

## 常见问题

**Q: 我的图片是列表，应该用哪个节点？**
A: 使用 **多视角图片批量输入** 节点，它可以直接接受图片列表。

**Q: 图片数量必须是固定的吗?**
A: 不是。批量输入支持任意数量；单个输入支持 1-8 张。建议至少 5 张以获得更好的 3D 效果。
